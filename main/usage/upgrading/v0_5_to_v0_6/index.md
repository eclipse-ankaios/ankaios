...
Up to v0.5 Ankaios only retried for 20 times in equal intervals to start a workload before marking it as failed to start. With the eventual consistency feature in v0.6, Ankaios is now indefinitely retrying to start the workload. To prevents overwhelming a service and reduce contention, the retries are done with exponential backoff and jitter.

...
pub fn validate_safe_name(name: &str) -> Result<String, PathSecurityError> {
    ...
    // Reject dotfiles and current directory references
    if name.starts_with('.') {
        return Err(PathSecurityError::InvalidCharacters(name.to_string()));
    }
...
mod tests {
    ...
    #[test]
    fn test_validate_safe_name_rejects_dotfiles() {
        assert!(validate_safe_name(".hidden").is_err());
        assert!(validate_safe_name(".env").is_err());
        assert!(validate_safe_name(".ssh").is_err());
    }

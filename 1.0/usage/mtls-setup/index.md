...
The standard [installation](https://eclipse-ankaios.github.io/ankaios/1.0/usage/installation/index.md) **disables mTLS** for server, agent and CLI in the respective [configuration files](https://eclipse-ankaios.github.io/ankaios/1.0/reference/config-files/index.md) with `insecure = true`. Besides that, mTLS can also be disabled for the CLI as command argument like `ank --insecure get state` or setting the environment variable `export ANK_INSECURE=true`.
...
For the agent add the following lines to `/etc/ankaios/ank-agent.conf` and set the `insecure` flag inside it to `false`:
...
For more information on how the server, agent and CLI can be configured please consult [configuration files](https://eclipse-ankaios.github.io/ankaios/1.0/reference/config-files/index.md).

Start the Ankaios server and an Ankaios agent as described in the [Quickstart](https://eclipse-ankaios.github.io/ankaios/1.0/usage/quickstart/index.md) and continue below to configure the CLI with mTLS.

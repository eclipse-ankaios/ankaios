...
The Ankaios agent `agent_A` will now instruct podman to start the workload. The command waits until the databroker is running. It should finally print:
...
The currently connected Ankaios agents are part of the CompleteState and can also be retrieved [working with the CompleteState](https://eclipse-ankaios.github.io/ankaios/1.0/reference/complete-state/index.md).
...
Note that this time the image does not specify the agent. While we could add `agent: infotainment`, this time we pass the agent name when the workload starts:
...
The speed-consumer workload subscribes to the vehicle speed signal and prints it to stdout. Use the web UI of the speed-provider to send a few vehicle speed values and watch the log messages of the speed-consumer with:
...
Previously we have used `ank get workloads` to a get list of running workloads. Ankaios also maintains a current state which can be retrieved with:
...
If we want to start the three workloads on startup of the Ankaios server and agents we need to create a startup manifest file. In the default server config (see `/etc/ankaios/ank-server.conf`), a startup manifest is read from `/etc/ankaios/state.yaml`.

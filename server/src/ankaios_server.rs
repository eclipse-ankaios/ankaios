...
use ankaios_api::ank_base::{
    ...
    RequestContentSpec, RequestSpec, WorkloadInstanceNameSpec, WorkloadStateSpec,
...
mod tests {
    ...
    use ankaios_api::ank_base::{
        CompleteState, CompleteStateRequestSpec, CompleteStateSpec, DeletedWorkload, Error,
        ExecutionStateEnumSpec, ExecutionStateSpec, LogEntriesResponse, LogEntry,
        LogsCancelAccepted, LogsRequestAccepted, LogsRequestSpec, LogsStopResponse,
    ...
    async fn utest_server_start_fail_on_invalid_startup_manifest() {
        ...
        let startup_state = CompleteStateSpec {
            desired_state: StateSpec {
                workloads: WorkloadMapSpec {
                    workloads: HashMap::from([(vars::WORKLOAD_NAMES[0].to_string(), workload)]),
                },
                ..Default::default()
            },
            ..Default::default()
        ...
        mock_server_state
            ...
            .return_const(Err(UpdateStateError::CycleInDependencies(
                vars::WORKLOAD_NAMES[0].to_string() + " part of cycle.",
            )));
    ...
    async fn utest_server_start_with_valid_startup_manifest() {
        ...
        assert_eq!(
            server
                .workload_states_map
                .get_workload_state_for_agent(vars::AGENT_NAMES[0]),
    ...
    async fn utest_server_sends_workloads_and_workload_states() {
        ...
        mock_server_state
            ...
            .with(mockall::predicate::eq(vars::AGENT_NAMES[0].to_string()))
            .once()
            .in_sequence(&mut seq)
        ...
        mock_server_state
            .expect_add_agent()
            .with(predicate::eq(vars::AGENT_NAMES[0].to_owned()))
            .once()
            .in_sequence(&mut seq)
            .return_const(());
        ...
        mock_server_state
            .expect_get_workloads_for_agent()
            .with(mockall::predicate::eq(vars::AGENT_NAMES[1].to_string()))
            .once()
            .in_sequence(&mut seq)
        ...
        mock_server_state
            .expect_add_agent()
            .with(predicate::eq(vars::AGENT_NAMES[1].to_owned()))
            .once()
            .in_sequence(&mut seq)
            .return_const(());
        ...
        let agent_hello_result = to_server
            .agent_hello(vars::AGENT_NAMES[0].to_string())
            .await;
        ...
        assert_eq!(
            FromServer::ServerHello(ServerHello {
                agent_name: Some(vars::AGENT_NAMES[0].to_string()),
        ...
        let agent_hello_result = to_server.agent_hello(vars::AGENT_NAMES[1].to_owned()).await;
        assert!(agent_hello_result.is_ok());
        ...
        assert_eq!(
            FromServer::ServerHello(ServerHello {
                agent_name: Some(vars::AGENT_NAMES[1].to_string()),
        ...
        let test_wl_2_state_succeeded =
            generate_test_workload_state(vars::WORKLOAD_NAMES[1], ExecutionStateSpec::succeeded());
        let update_workload_state_result = to_server
    ...
    async fn utest_server_sends_workloads_and_workload_states_when_requested_update_state_success()
    {
        ...
        let update_state_result = to_server
            .update_state(vars::REQUEST_ID.to_string(), update_state, update_mask)
            .await;
    ...
    async fn utest_server_sends_workloads_and_workload_states_when_requested_update_state_nothing_to_do()
     {
        ...
        assert!(matches!(
            ...
            }) if request_id == vars::REQUEST_ID && added_workloads.is_empty() && deleted_workloads.is_empty()
        ));
    ...
    async fn utest_server_sends_workloads_and_workload_states_when_requested_update_state_error() {
        ...
        let w1 = generate_test_workload_with_params(vars::AGENT_NAMES[0], vars::RUNTIME_NAMES[0]);

        let update_state = CompleteStateSpec {
            desired_state: StateSpec {
                workloads: WorkloadMapSpec {
                    workloads: HashMap::from([(vars::WORKLOAD_NAMES[0].to_owned(), w1.clone())]),
                },
                ..Default::default()
            },
            ..Default::default()
    ...
    async fn utest_server_forward_logs_request_to_agents() {
        ...
        server
            ...
            .with(
                predicate::eq(vars::REQUEST_ID.to_owned()),
        ...
        let logs_request_result = to_server
            .logs_request(vars::REQUEST_ID.to_string(), logs_request.into())
            .await;
            ...
            FromServer::Response(Response {
                ...
                response_content: Some(ResponseContent::LogsRequestAccepted(LogsRequestAccepted {
                    workload_names: vec![WorkloadInstanceName {
                        workload_name: vars::WORKLOAD_NAMES[0].to_string(),
                        agent_name: vars::AGENT_NAMES[0].to_string(),
                        id: vars::WORKLOAD_IDS[0].to_string()
                    }],
                })),
            })
        );
    ...
    async fn utest_server_returns_complete_state_when_received_request_complete_state() {
        ...
        let w2 = w1.clone();
        let w3 = Workload {
            agent: Some(vars::AGENT_NAMES[1].to_string()),
            ..w1.clone()
        };

        let workloads = HashMap::from([
            (vars::WORKLOAD_NAMES[0].to_owned(), w1),
            (vars::WORKLOAD_NAMES[1].to_owned(), w2),
            (vars::WORKLOAD_NAMES[2].to_owned(), w3),
        ]);
        ...
        let request_id = format!("{}@my_request_id", vars::AGENT_NAMES[0]);
        let mut server = AnkaiosServer::new(server_receiver, to_agents);
        let mut mock_server_state = MockServerState::new();
        mock_server_state
    ...
    async fn utest_server_start_distributes_workload_states_after_agent_disconnect() {
        ...
        mock_server_state
            .expect_remove_agent()
            .once()
            .with(predicate::eq(vars::AGENT_NAMES[0]))
            .return_const(());
        ...
        let agent_gone_result = to_server.agent_gone(vars::AGENT_NAMES[0].to_owned()).await;
    ...
    async fn utest_server_sends_logs_cancel_requests_on_disconnected_agent() {
        ...
        let expected_logs_cancel_requests = vec![
            FromServer::LogsCancelRequest(vars::REQUEST_ID.to_string()),
    ...
    async fn utest_server_start_calls_agents_in_update_state_command() {
        ...
        let agent_hello1_result = to_server.agent_hello(vars::AGENT_NAMES[0].to_owned()).await;
        assert!(agent_hello1_result.is_ok());

        let agent_hello2_result = to_server.agent_hello(vars::AGENT_NAMES[1].to_owned()).await;
        ...
        assert!(matches!(
            ...
            }) if request_id == vars::REQUEST_ID && added_workloads == vec![updated_w1.instance_name.to_string()] && deleted_workloads == vec![w1.instance_name.to_string()]
        ));
    ...
    async fn utest_server_log_cancel_request() {
        ...
        let request_id = vars::REQUEST_ID.to_string();
        let mut server = AnkaiosServer::new(server_receiver, to_agents);
        server
            .log_campaign_store

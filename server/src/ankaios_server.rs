...
impl AnkaiosServer {
    ...
    pub async fn start(&mut self, startup_state: Option<CompleteState>) -> Result<(), String> {
        if let Some(state) = startup_state {
            ...
            let startup_state: Object = state
                .try_into()
                .map_err(|err| format!("Could not parse into CompleteState: '{err}'"))?;

            match self
                .server_state
                .update(current_state, startup_state, vec![])
            {
        ...
        while let Some(to_server_command) = self.receiver.recv().await {
            match to_server_command {
                ToServer::AgentHello(method_obj) => {
                    ...
                    self.agents.add_agent(agent_name);
                }
                ...
                ToServer::AgentLoadStatus(method_obj) => {
                    ...
                    self.agents.update_agent_resource_availability(method_obj);
                }
                ToServer::AgentGone(method_obj) => {
                    ...
                    self.agents.remove_agent(&agent_name);
                ...
                ToServer::Request(Request {
                    ...
                    common::commands::RequestContent::UpdateStateRequest(update_state_request) => {
                        ...
                        let current_state: Object = match self.server_state.get_state().try_into() {
                            Ok(state) => state,
                            Err(err) => {
                                let error_msg = format!("Failed to parse current state, '{err}'");
                        ...
                        let state_from_update: Object = match update_state_request.state.try_into()
                        {
                            Ok(state) => state,
                            Err(err) => {
                                let error_msg =
                                    format!("Update rejected: Failed to parse new state, '{err}'");
                                log::error!("Update rejected: '{error_msg}'");
                                self.to_agents
                                    .error(request_id, error_msg)
                                    .await
                        ...
                        let mut state_differences =
                            current_state.calculate_state_differences(&state_from_update);
                        ...
                        println!("State from update: {state_from_update:?}");
...
mod tests {
    ...
    use common::{from_server_interface::FromServer, state_manipulation::Object};
    ...
    async fn utest_server_start_fail_on_invalid_startup_manifest() {
        ...
        let old_state = CompleteState::default();
        let old_state_obj: Object = old_state.clone().try_into().unwrap_or_unreachable();
        let startup_state_obj: Object = startup_state.clone().try_into().unwrap_or_unreachable();

        mock_server_state.expect_get_state().return_const(old_state);

        mock_server_state
            .expect_update()
            .with(
                mockall::predicate::eq(old_state_obj),
                mockall::predicate::eq(startup_state_obj),
    ...
    async fn utest_server_update_state_continues_on_invalid_new_state() {
        ...
        let current_state = CompleteState::default();
        let current_state_obj: Object = current_state.clone().try_into().unwrap_or_unreachable();
        let new_state_obj: Object = new_state.clone().try_into().unwrap_or_unreachable();

        mock_server_state
            .expect_get_state()
            .once()
            .in_sequence(&mut seq)
            .return_const(current_state.clone());

        mock_server_state
            .expect_update()
            .with(
                mockall::predicate::eq(current_state_obj),
                mockall::predicate::eq(new_state_obj.clone()),
        ...
        let fixed_state_obj: Object = fixed_state.clone().try_into().unwrap_or_unreachable();

        mock_server_state
            .expect_get_state()
            .once()
            .in_sequence(&mut seq)
            .return_const(new_state.clone());

        mock_server_state
            .expect_update()
            .with(
                mockall::predicate::eq(new_state_obj),
                mockall::predicate::eq(fixed_state_obj),
                mockall::predicate::eq(update_mask.clone()),
            )
            .once()
            .in_sequence(&mut seq)
    ...
    async fn utest_server_start_with_valid_startup_manifest() {
        ...
        let mut server = AnkaiosServer::new(server_receiver, to_agents);
        let mut mock_server_state = MockServerState::new();
        let empty_default_state = CompleteState::default();
        let empty_default_state_obj: Object = empty_default_state
            .clone()
            .try_into()
            .unwrap_or_unreachable();
        ...
        mock_server_state
            .expect_get_state()
            .return_const(empty_default_state.clone());

        let startup_state_obj: Object = startup_state.clone().try_into().unwrap_or_unreachable();
        mock_server_state
            .expect_update()
            .with(
                mockall::predicate::eq(empty_default_state_obj),
                mockall::predicate::eq(startup_state_obj),
                mockall::predicate::eq(vec![]),
            )
            .once()
    ...
    async fn utest_server_sends_workloads_and_workload_states_when_requested_update_state_success()
    {
        ...
        let mut mock_server_state = MockServerState::new();
        let old_state = CompleteState::default();
        let empty_default_state_obj: Object = old_state.clone().try_into().unwrap_or_unreachable();
        let update_state_obj: Object = update_state.clone().try_into().unwrap_or_unreachable();

        mock_server_state
            .expect_get_state()
            .once()
            .return_const(old_state);
        mock_server_state
            .expect_update()
            .with(
                mockall::predicate::eq(empty_default_state_obj),
                mockall::predicate::eq(update_state_obj),
                mockall::predicate::eq(update_mask.clone()),
            )
            .once()
            .return_const(Ok(Some((
    ...
    async fn utest_server_sends_workloads_and_workload_states_when_requested_update_state_error() {
        ...
        let update_state_obj: Object = update_state.clone().try_into().unwrap_or_unreachable();
        println!("utest update_state_obj: {update_state_obj:?}");
    ...
    async fn utest_server_start_calls_agents_in_update_state_command() {
        ...
        mock_server_state
            ...
            .once()
            .in_sequence(&mut seq)
            .return_const(old_state.clone());

        mock_server_state
            .expect_update()
            .with(
                mockall::predicate::eq::<Object>(old_state.try_into().unwrap_or_unreachable()),
                mockall::predicate::eq::<Object>(
                    update_state.clone().try_into().unwrap_or_unreachable(),
                ),
                mockall::predicate::eq(update_mask.clone()),
            )
            .once()
        ...
        let old_state = CompleteState {
            desired_state: State {
                workloads: HashMap::from([
                    (
                        WORKLOAD_NAME_1.to_owned(),
                        workload_without_agent.clone().into(),
                    ),
                    (
                        WORKLOAD_NAME_2.to_owned(),
                        workload_with_agent.clone().into(),
                    ),
                ]),
                ..Default::default()
            },
                ...
                desired_state: State {
                    workloads: HashMap::from([(
                        log_collecting_workload
                            ...
                            .workload_name()
                            .to_owned(),
                        log_collecting_workload.clone().into(),
                    )]),
                    ..Default::default()
                },
                ..Default::default()
    ...
    async fn utest_server_receives_agent_status_load() {
        ...
        let mut server = AnkaiosServer::new(server_receiver, to_agents);
        server.agents.add_agent(AGENT_A.to_owned());

        let agent_resource_result = to_server.agent_load_status(payload.clone()).await;
        ...
        let mut expected_agent_map = common::objects::AgentMap::default();
        expected_agent_map.add_agent(AGENT_A.to_owned());
        expected_agent_map.update_agent_resource_availability(payload);

        assert_eq!(server.agents, expected_agent_map);
    }

...
impl SignatureValidator {
    ...
    pub fn from_keys_directory(
        ...
        let counter_state_path = match std::env::var("ANKAIOS_COUNTER_STATE_PATH") {
            Ok(path_str) => {
                ...
                match Self::validate_counter_state_path(&path, strict_security) {
                    Ok(_) => path,
                    Err(e) => {
    ...
    fn validate_counter_state_path(path: &Path, strict_security: bool) -> Result<(), SignatureError> {
        // Use configured strict security mode
        let strict_mode = strict_security;
    ...
    fn validate_and_update_counter(
        ...
        if let Some(&last_seen) = self.key_counters.get(key_id) {
            ...
            if is_restoration {
                // Use configured value (already overridden by env var at startup if present)
                let restoration_window = self.restoration_window_seconds;
...
// [utest->swdd~server-signature-validation~1]
// [utest->swdd~server-signature-canonical-protobuf~1]
// [utest->swdd~server-signature-includes-counter-timestamp~1]
// [utest->swdd~server-signature-replay-protection~1]
// [utest->swdd~server-signature-constant-time~1]
// [utest->swdd~server-signature-security-model~1]
#[cfg(test)]
mod tests {
    use super::*;

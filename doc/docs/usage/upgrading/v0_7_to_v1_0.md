...
This documents describes the necessary changes required to update to the new stable version of Ankaios.

For a complete list of new features, please consult the [official release notes](https://github.com/eclipse-ankaios/ankaios/releases/tag/v1.0.0).
...
This improves the accessibility by allowing to filter the complete state for a distinct tag:

```bash
ank get state desiredState.workloads.*.tags.owner
```

Backwards compatibility to the old specification is still provided by `ank-server` and `ank` CLI when reading manifests, but the CLI will not output in the old format anymore.
...
Also here backwards compatibility is maintained and the old manifests are still accepted.
...
Ankaios agents now support tags which can be set either at agent startup or dynamically during runtime via the CLI or the Control Interface. The tags can specify agent attributes which can be read, for example, by scheduling plugins.
...
Our SDKs now support the v1.0 too and also provide a stable release:

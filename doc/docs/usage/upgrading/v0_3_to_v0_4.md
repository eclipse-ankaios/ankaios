...
Starting from v0.4.0, Ankaios requires an explicit authorization in order to provide workloads access to the Control Interface. The authorization is based on allow and deny rules that pinpoint what a workload can or cannot do. Without explicitly specifying an allow rule, a workload is not allowed to do anything with the control interface. Because of this, existing workloads that use the Control Interface must be explicitly allowed to access it.

The following configuration shows an example of a ControlInterfaceAccess that allows updating and reading all workloads, but denies updating the workload `watchdog`:

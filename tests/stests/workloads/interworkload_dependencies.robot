...
*** Test Cases ***
# [stest->swdd~agent-handles-new-workload-operations]
...
# [stest->swdd~agent-handles-new-workload-operations]
# [stest->swdd~agent-enqueues-unfulfilled-delete~1]
...
# [stest->swdd~agent-handles-new-workload-operations]
# [stest->swdd~agent-enqueues-update-with-unfulfilled-delete~1]
# [stest->swdd~agent-handles-workloads-with-fulfilled-dependencies~1]
...
# [stest->swdd~agent-handles-new-workload-operations]
# [stest->swdd~agent-handles-update-with-fulfilled-delete~1]
# [stest->swdd~agent-handles-workloads-with-fulfilled-dependencies~1]
Test Ankaios CLI update workload with pending create
    ...
    # Actions
    When user triggers "ank -k --no-wait set state desiredState.workloads.after_backend ${new_state_yaml_file}"
    And the workload "after_backend" shall have the execution state "Pending(WaitingToStart)" on agent "agent_A" within "3" seconds
    And user triggers "ank -k set state desiredState.workloads.backend ${new_state_yaml_file}"
    # Asserts

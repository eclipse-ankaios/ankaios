...
Test Ankaios Podman create workloads
    ...
    And podman shall have a container for workload "sleepy" on agent "agent_A"
...
Test Ankaios Podman create a container with custom name
    ...
    # This test assumes that all containers in the podman have been created with this test -> clean it up first
    Given Podman has deleted all existing containers
    And Ankaios server is started with manifest "${CONFIGS_DIR}/create_workload_custom_name.yaml"
    And the CLI listens for workload states
    # Actions
    When Ankaios agent is started with name "agent_A"
    # Asserts
    Then the workload "sleepy" shall have the execution state "Running(Ok)" on agent "agent_A" within "20" seconds
    And podman shall have a container for workload "sleepy" with custom name "test_workload1" on agent "agent_A"
    [Teardown]    Clean up Ankaios

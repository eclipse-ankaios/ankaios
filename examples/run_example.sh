...
DEFAULT_MANIFEST_FILE="startupState.yaml"

display_usage() {
    echo -e "Usage: $0 <name> [--manifest-file <manifest_file>] [--ankaios-bin-dir <path>] [additional podman build args]"
    echo -e ""
    ...
    echo -e "  --manifest-file:   specify a manifest file from the config directory to use. Default is '${DEFAULT_MANIFEST_FILE}'."
...
run_ankaios() {
  ...
  RUST_LOG=debug ${ANK_BIN_DIR}/ank-server --insecure --startup-config ${SCRIPT_DIR}/config/${MANIFEST_FILE} --address ${ANKAIOS_SERVER_SOCKET} > ${ANKAIOS_LOG_DIR}/ankaios-server.log 2>&1 &
...
podman build ${PODMAN_BUILD_ARGS} -t control_interface_prod:0.1 -f examples/"${EXAMPLE}"/Dockerfile "${SCRIPT_DIR}"/../
echo done.

...
async fn persist_workload(
    ...
    let temp_file = safe_join(&workloads_dir, &format!(".{}.{}.tmp", workload_name, extension))
        .map_err(|e| format!("Invalid workload name '{}': {}", workload_name, e))?;
    ...
    #[cfg(unix)]
    {
        ...
        let mut perms = tokio::fs::metadata(&workload_file).await?.permissions();
        perms.set_mode(0o600);
        tokio::fs::set_permissions(&workload_file, perms).await?;
    }
...
async fn process_event(
        ...
        if let Some(workload_name) = field.strip_prefix("desiredState.workloads.") {
            ...
            if let Some(mode) = get_persist_mode(&workload.tags) {
                ...
                match mode.as_str() {
                    "ALWAYS" => {
                        // Add immediately
                        log::info!("✅ Adding workload '{}' with persist: ALWAYS to persisted_state", workload_name);
                        add_workload_to_persisted_state(&mut persisted_state, workload_name, workload);
                        modified = true;
                        log::info!("✅ modified flag set to true");
                    }
                    "ON_RUNNING" => {
    ...
    for field in &altered_fields.removed_fields {
        if let Some(workload_name) = field.strip_prefix("desiredState.workloads.") {
            ...
            if remove_workload_from_persisted_state(&mut persisted_state, workload_name) {
                modified = true;
                log::info!("🗑️  Removed '{}' from in-memory state", workload_name);
            }
                                    ...
                                    match mode.as_str() {
                                        "ALWAYS" => {
                                            ...
                                            if let Err(e) = persist_workload(workload_name, workload, signed_request, &workloads_dir).await {
                                                ...
                                                add_workload_to_persisted_state(&mut persisted_state, workload_name, workload);
                                                // Note: we don't set modified=true here because we already persisted directly
                                            }
                                        }
                                        "ON_RUNNING" => {
    ...
    // Write file for ALWAYS workloads (modified = true)
    // ON_RUNNING workloads are persisted directly when they reach Running state
    if modified {
        log::info!("💾 modified=true, persisting ALWAYS workloads");
        // Persist workloads that were added to persisted_state (ALWAYS workloads)
        if let Some(ref complete_state) = response.complete_state {
            if let Some(ref desired_state) = complete_state.desired_state {
                let workloads_dir = get_workloads_dir(persistence_file);
                log::info!("💾 workloads_dir: {:?}", workloads_dir);

                // Iterate over workloads in persisted_state (only ALWAYS workloads that were just added)
                if let Some(ref persisted_workloads) = persisted_state.workloads {
                    log::info!("💾 persisted_workloads count: {}", persisted_workloads.workloads.len());
                    for workload_name in persisted_workloads.workloads.keys() {
                        log::info!("💾 Persisting workload: {}", workload_name);

                        // Get the workload from desired_state
                        if let Some(workload) = desired_state.workloads.as_ref()
                            .and_then(|ws| ws.workloads.get(workload_name)) {

                            // Check if this workload has an original signed request
                            let signed_request = response.signed_workload_requests
                                .get(workload_name);

                            persist_workload(workload_name, workload, signed_request, &workloads_dir).await?;
                        }
                    }
                }
            }
        } else {
            log::warn!("ALWAYS workload modified but no complete_state in event");
        }
    } else {

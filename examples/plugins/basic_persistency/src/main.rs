...
async fn process_event(
    ...
    for field in &altered_fields.added_fields {
        ...
        // Only process top-level workload additions (exactly "desiredState.workloads.name")
        if parts.len() == 3 {
    ...
    for field in &altered_fields.removed_fields {
        ...
        let parts: Vec<&str> = field.split('.').collect();
        // Only process top-level workload removals (exactly "desiredState.workloads.name")
        if parts.len() == 3 {
            let workload_name = parts[2];

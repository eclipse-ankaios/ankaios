...
async fn persist_workload(
    ...
    // File path: /var/lib/ankaios/workloads/<workload_name>.yaml
    ...
    let temp_file = workloads_dir.join(format!(".{}.yaml.tmp", workload_name));
            ...
            if let Some(mode) = get_persist_mode(&workload.tags) {
                ...
                match mode.as_str() {
                    ...
                    "ON_RUNNING" => {
                        ...
                        // Cache workload for later when the workload reaches Running state
                            ...
                            if let Some(workload) = workloads.workloads.get(workload_name) {
                                if let Some(mode) = get_persist_mode(&workload.tags) {
                                    match mode.as_str() {
                                        ...
                                        "ON_RUNNING" => {
                                            // Update the cached workload for ON_RUNNING workloads
                                            log::debug!("Workload '{}' (ON_RUNNING) updated, updating cached state", workload_name);

                                            // Cache the workload for later persistence when Running
                                            on_running_cached_workloads.insert(workload_name.to_string(), workload.clone());
                                        }
                                        _ => {}
                                    }
...
/// Currently always returns None since YAML files don't have signature counters.
fn extract_signature_counter(_file_path: &Path) -> Option<u64> {
    None
}
        ...
        match message.from_ankaios_enum {
            Some(FromAnkaiosEnum::Response(response)) => {
                ...
                if response.request_id == EVENT_REQUEST_ID {
                    ...
                    {
                        // Process the event
                        if let Err(e) = process_event(&state_response, &persistence_file, &mut output_pipe, &mut input_pipe, &mut on_running_workloads, &mut on_running_cached_workloads, &mut persisted_workload_names).await {
...
mod tests {
    ...
    async fn test_persist_workload() {
        let temp_dir = std::env::temp_dir();
        let temp_workloads_dir = temp_dir.join(format!("test_persist_{}_workloads", unique_test_id()));
    ...
    async fn test_persist_workload_cleans_temp_on_failure() {
        let temp_dir = std::env::temp_dir().join(format!("test_temp_cleanup_{}", unique_test_id()));
        ...
        let temp_file = workloads_dir.join(".test-workload.yaml.tmp");
        assert!(!temp_file.exists(), "Temp file should be cleaned up on failure");

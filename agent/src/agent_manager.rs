...
use common::execution_interface::{ExecutionCommand, ExecutionReceiver};
        ...
        while let Some(x) = self.receiver.recv().await {
            match x {
                ...
                ExecutionCommand::UpdateWorkloadState(method_obj) => {
                    ...
                    let _ = self
                        .state_sink
                        .store_remote_workload_states(method_obj.workload_states)
                        .await;
                }
...
mod tests {
    use super::*;
    use crate::{agent_manager::AgentManager, workload_state::WorkloadStateMessage};
    use common::{
    ...
    async fn utest_agent_manager_update_workload() {
        ...
        let (state_sink, _state_change_rx) =
    ...
    async fn utest_agent_manager_update_workload_states() {
        ...
        let (state_sink, mut state_change_rx) =
            tokio::sync::mpsc::channel::<WorkloadStateMessage>(TEST_CHANNEL_BUFFER_SIZE);

        // TODO: check the receiver
        ...
        let x = state_change_rx.recv().await.unwrap();
        assert!(matches!(
            x,
            WorkloadStateMessage::FromServer(state) if state[0].execution_state == ExecutionState::ExecRunning
        ));

        let y = state_change_rx.try_recv();
        assert!(y.is_err());
    }
    ...
    async fn utest_agent_manager_forwards_complete_state() {
        ...
        // the expectation is tha the mock runtime manager will receive the complete state
    }
}

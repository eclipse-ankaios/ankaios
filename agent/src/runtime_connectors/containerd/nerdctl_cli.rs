...
mod tests {
    ...
    async fn utest_run_container_success_no_options() {
        ...
        super::CliCommand::new_expect(
            ...
            super::CliCommand::default()
                ...
                .exec_returns(Ok(vars::WORKLOAD_IDS[0].to_owned())),
        );
        ...
        .await;
        assert_eq!(res, Ok(vars::WORKLOAD_IDS[0].to_owned()));
    }
    ...
    async fn utest_start_container_success() {
        ...
        super::CliCommand::new_expect(
            NERDCTL_CMD,
            super::CliCommand::default()
                .expect_args(&["--remote", "start", vars::WORKLOAD_IDS[0]])
                .exec_returns(Ok(vars::WORKLOAD_IDS[0].to_owned())),
        );

        let start_config = super::NerdctlStartConfig {
            general_options: vec!["--remote".into()],
            container_id: vars::WORKLOAD_IDS[0].into(),
        };
    ...
    async fn utest_list_states_by_id_created() {
        ...
        super::CliCommand::new_expect(
            NERDCTL_CMD,
            super::CliCommand::default()
                .expect_args(&["inspect", vars::WORKLOAD_IDS[0]])
        ...
        let res = NerdctlCli::list_states_by_id(vars::WORKLOAD_IDS[0]).await;
    ...
    async fn utest_remove_workloads_by_id_stop_failed() {
        ...
        super::CliCommand::new_expect(
            NERDCTL_CMD,
            super::CliCommand::default()
                .expect_args(&["stop", vars::WORKLOAD_IDS[0]])
                .exec_returns(Err("simulated error".to_string())),
        );

        assert_eq!(
            NerdctlCli::remove_workloads_by_id(vars::WORKLOAD_IDS[0]).await,
    ...
    async fn utest_remove_workloads_by_id_ignore_failed_stop_on_non_existing_container() {
        ...
        super::CliCommand::new_expect(
            ...
            super::CliCommand::default()
                .expect_args(&["stop", vars::WORKLOAD_IDS[0]])
                .exec_returns(Err(format!("1 errors:\nno such container: {}", vars::WORKLOAD_IDS[0]))),
        );

        super::CliCommand::new_expect(
            NERDCTL_CMD,
            super::CliCommand::default()
                .expect_args(&["rm", vars::WORKLOAD_IDS[0]])
                .exec_returns(Ok(vars::WORKLOAD_IDS[0].to_owned())),
        );

        assert!(
            NerdctlCli::remove_workloads_by_id(vars::WORKLOAD_IDS[0])
                .await
                .is_ok(),
    ...
    async fn utest_remove_workloads_by_id_success() {
        ...
        let res = NerdctlCli::remove_workloads_by_id(vars::WORKLOAD_IDS[0]).await;
        assert_eq!(res, Ok(()));
    }

...
mod tests {
    ...
    const DECODED_BINARY_DATA: &str = "base64 data"; // decoded vars::FILE_BINARY_DATA
    ...
    async fn utest_workload_files_creator_create_files() {
        ...
        let workload_files = vec![
            ...
            FileSpec {
                ...
                file_content: FileContentSpec::Data {
                    data: vars::FILE_TEXT_DATA.to_owned(),
                },
            },
            // Binary file
            FileSpec {
                ...
                file_content: FileContentSpec::BinaryData {
                    binary_data: vars::FILE_BINARY_DATA.to_owned(), // "data" as base64
                },
            },
        ];
        ...
        mock_write_file_context
            ...
            .with(
                ...
                predicate::eq(vars::FILE_TEXT_DATA.to_owned()),
            )
            .returning(|_, _: String| Ok(()));

        let binary_file_path =
            workload_files_path.join(vars::FILE_BINARY_PATH.trim_start_matches('/'));
        mock_write_file_context
            .expect()
            .once()
            .with(
                predicate::eq(binary_file_path.clone()),
                predicate::eq(DECODED_BINARY_DATA.to_owned().as_bytes().to_vec()),
            )
            .returning(|_, _: Vec<u8>| Ok(()));

        let expected_host_file_paths = HashMap::from([
            (text_host_file_path, PathBuf::from("/some/path/test.conf")),
            (binary_file_path, PathBuf::from(vars::FILE_BINARY_PATH)),
        ]);
        assert_eq!(
    ...
    async fn utest_workload_files_creator_create_files_decode_wrapped_base64_input() {
        ...
        let binary_file_path = workload_files_path.join(vars::FILE_BINARY_PATH.trim_start_matches('/'));
        let mock_write_file_context = mock_filesystem_async::write_file_context();
        ...
        let expected_host_file_paths =
            HashMap::from([(binary_file_path, PathBuf::from(vars::FILE_BINARY_PATH))]);
